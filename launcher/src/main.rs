...
// ⚾ 野球のバットとボールを描画する関数
fn draw_baseball_icon(x: i32, y: i32, is_selected: bool) {
    let bat_color = if is_selected { (255, 160, 0) } else { (160, 110, 50) };
    let ball_color = if is_selected { (255, 255, 255) } else { (180, 185, 190) };
    
    // 対角線上の野球バットを描画
    for i in 0..16 {
        let bx = x + 4 + i * 2;
        let by = y + 34 - i * 2;
        let w = if i < 4 { 4 } else if i < 10 { 6 } else { 8 };
        runtime_draw_rect(bx, by, w, w, bat_color.0, bat_color.1, bat_color.2);
    }
    
    // ボールを描画
    let ball_x = x + 28;
    let ball_y = y + 6;
    runtime_draw_rect(ball_x + 3, ball_y, 6, 12, ball_color.0, ball_color.1, ball_color.2);
    runtime_draw_rect(ball_x, ball_y + 3, 12, 6, ball_color.0, ball_color.1, ball_color.2);
    runtime_draw_rect(ball_x + 1, ball_y + 1, 10, 10, ball_color.0, ball_color.1, ball_color.2);
    
    // 縫い目 (赤)
    runtime_draw_rect(ball_x + 3, ball_y + 3, 2, 2, 255, 50, 50);
    runtime_draw_rect(ball_x + 7, ball_y + 7, 2, 2, 255, 50, 50);
}
...
fn main() {
    ...
    while is_running {
        ...
        for (i, game) in games.iter().enumerate() {
            ...
            if i == selected_index {
                // 選択されているゲーム：発光カラーをネオンブルー〜シアンで美しくパルス
                let r_pulse = (100.0 + (animation_time.sin() * 50.0)) as u8;
                let g_pulse = (220.0 + (animation_time.cos() * 35.0)) as u8;
                let b_pulse = 255;

                // 光り輝く選択ネオン枠
                runtime_draw_rect(43, item_y - 2, 1194, 104, r_pulse, g_pulse, b_pulse);
                // 選択インジケータ (ブレなく綺麗に表示)
                runtime_draw_rect(55, item_y + 10, 12, 80, 255, 255, 255);
                // ゲームカード内部 (プレミアムダークブルー)
                runtime_draw_rect(80, item_y + 5, 1145, 90, 25, 30, 50);
            } else {
            ...
            // アイコンエリアの描画 (静的でくっきり描画)
            let icon_x = 105;
            let icon_y = item_y + 25;
            
            // 各ゲームに応じた美しいカスタムピクセルアートを表示！
            match game.meta.icon.as_str() {
                "controller" => draw_controller_icon(icon_x, icon_y, i == selected_index),
                "baseball" => draw_baseball_icon(icon_x, icon_y, i == selected_index),
                "spaceship" => draw_spaceship_icon(icon_x, icon_y, i == selected_index),
                _ => {
                    // フォールバック：メタデータにアイコン設定がない場合
                    if game.meta.exec == "sample_game" {
                        draw_controller_icon(icon_x, icon_y, i == selected_index);
                    } else if game.meta.exec == "baseball_training" {
                        draw_baseball_icon(icon_x, icon_y, i == selected_index);
                    } else {
            ...
            // ゲームタイトルとバージョンを描画 (静的で完全にブレのないクッキリ表示)
            let text_x = 180;
